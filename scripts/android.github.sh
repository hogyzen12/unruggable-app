...
echo "Extracting universal APK from APKS..."

# Define output APK path
OUTPUT_APK="./dist/android/${APP_NAME}-universal.apk"

# Create a temp directory for extraction
TEMP_DIR="./dist/android/temp_extract"
mkdir -p "$TEMP_DIR"

# Unzip the APKS (it's a ZIP file)
unzip "$OUTPUT_APKS" -d "$TEMP_DIR"

# Move the universal.apk to the desired location
if [ -f "$TEMP_DIR/universal.apk" ]; then
  mv "$TEMP_DIR/universal.apk" "$OUTPUT_APK"
  echo "Universal APK extracted successfully at $OUTPUT_APK"
else
  echo "Error: universal.apk not found in APKS archive."
  exit 1
fi
