// Liquid Metal Circle Border Component - WebGL border shader for circular elements
window.LiquidMetalCircleBorder = (function() {
  'use strict';

  class CircleBorderShader {
    ...
    initCanvas() {
      ...
      this.canvas.className = 'liquid-metal-circle-border-canvas';
      ...
      console.log('Circle border canvas created:', this.canvas.width, 'x', this.canvas.height);
    }
    ...
    initShader() {
      ...
      // Fragment shader optimized for circular borders
      const fragmentShader = `#version 300 es
        ...
        void main() {
          ...
          // Only render in circle border region
          float outerEdge = radius;
          float innerEdge = radius - u_borderWidth;
          
          if (dist > outerEdge || dist < innerEdge) {
            discard;
          }
          ...
          // Anti-aliased circular edges
          float edgeSmoothness = 1.5;
          float outerAlpha = smoothstep(outerEdge + edgeSmoothness, outerEdge - edgeSmoothness, dist);
          float innerAlpha = smoothstep(innerEdge - edgeSmoothness, innerEdge + edgeSmoothness, dist);
      ...
      console.log('Circle border shader initialized successfully');
    }
  ...
  return {
    create: function(elementId, options) {
      ...
      if (!element) {
        console.error('LiquidMetalCircleBorder: Element not found:', elementId);
        return null;
      }
      return new CircleBorderShader(element, options);
    },
    
    applyTo: function(element, options) {
      if (!element) {
        console.error('LiquidMetalCircleBorder: No element provided');
        return null;
      }
      return new CircleBorderShader(element, options);
    }
