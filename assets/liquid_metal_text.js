// Liquid Metal Text Component - WebGL shader for animated liquid metal text
window.LiquidMetalText = (function() {
  'use strict';

  class LiquidMetalTextShader {
    ...
    initCanvas() {
      ...
      const metrics = measureCtx.measureText(this.text);
      
      // Set container and canvas dimensions based on text
      const dpr = Math.min(window.devicePixelRatio || 1, 2);
      const width = Math.ceil(metrics.width) + 20; // padding
      const height = Math.ceil(this.fontSize * 1.4); // line height
      ...
      if (!this.gl) {
        console.error('WebGL2 not supported for LiquidMetalText');
        return;
      }
    }
    ...
    createTextTexture() {
      ...
      // Draw text
      const dpr = Math.min(window.devicePixelRatio || 1, 2);
      ctx.font = `${this.fontWeight} ${this.fontSize * dpr}px ${this.fontFamily}`;
      ctx.fillStyle = 'white';
      ctx.textBaseline = 'middle';
      ctx.textAlign = 'center';
      ctx.fillText(this.text, textCanvas.width / 2, textCanvas.height / 2);
      
      // Create WebGL texture from text canvas
    ...
    initShader() {
      ...
      // Fragment shader with liquid metal effect masked by text
      const fragmentShader = `#version 300 es
        ...
        void main() {
          ...
          float noise = snoise(uv * 2.0 - t * 0.5);
          
          float direction = grad_uv.x + diagBLtoTR;
          direction -= 2.0 * noise * diagBLtoTR * 0.3;
          ...
          // Use text mask for alpha with smooth edges
          float alpha = smoothstep(0.1, 0.5, textMask);
      ...
      if (!gl.getShaderParameter(vs, gl.COMPILE_STATUS)) {
        console.error('LiquidMetalText vertex shader error:', gl.getShaderInfoLog(vs));
      }
      ...
      if (!gl.getShaderParameter(fs, gl.COMPILE_STATUS)) {
        console.error('LiquidMetalText fragment shader error:', gl.getShaderInfoLog(fs));
      }
      ...
      if (!gl.getProgramParameter(this.program, gl.LINK_STATUS)) {
        console.error('LiquidMetalText program link error:', gl.getProgramInfoLog(this.program));
      }
      ...
      this.uTextTexture = gl.getUniformLocation(this.program, 'u_textTexture');
    }
    ...
    render = (time) => {
      ...
      gl.bindTexture(gl.TEXTURE_2D, this.textTexture);
      gl.uniform1i(this.uTextTexture, 0);
  ...
  return {
    create: function(containerId, options) {
      ...
      if (!container) {
        console.error('LiquidMetalText: Container not found:', containerId);
        return null;
      }
      return new LiquidMetalTextShader(container, options);
    },
    
    applyTo: function(container, options) {
      if (!container) {
        console.error('LiquidMetalText: No container provided');
        return null;
      }
      return new LiquidMetalTextShader(container, options);
    }
