...
use crate::components::common::{
    SortCriteria, Token, TokenCategory, TokenDisplayData, TokenFilter, TokenSortConfig,
};
...
fn get_sort_priority(symbol: &str, _category: &TokenCategory) -> u32 {
    match symbol {
        "SOL" => 0,  // Always first
...
pub fn sort_tokens(tokens: &mut Vec<TokenDisplayData>, config: &TokenSortConfig) {
    tokens.sort_by(|a, b| {
        ...
        let cmp = match config.primary {
            SortCriteria::ValueUsd => b
                .token
                .value_usd
                .partial_cmp(&a.token.value_usd)
                .unwrap_or(std::cmp::Ordering::Equal),
            SortCriteria::Balance => b
                .token
                .balance
                .partial_cmp(&a.token.balance)
                .unwrap_or(std::cmp::Ordering::Equal),
            SortCriteria::PriceChange24h => b
                .token
                .price_change_1d
                .partial_cmp(&a.token.price_change_1d)
                .unwrap_or(std::cmp::Ordering::Equal),
            SortCriteria::Alphabetical => a.token.symbol.cmp(&b.token.symbol),
