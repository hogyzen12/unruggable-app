...
fn convert_compute_budget_instruction(
    interface_ix: solana_instruction::Instruction,
) -> Instruction {
    Instruction {
        program_id: Pubkey::new_from_array(interface_ix.program_id.to_bytes()),
        accounts: interface_ix
            .accounts
            .iter()
...
fn build_instant_unstake_instruction(
    ...
    println!(
        "Building instant unstake instruction for stake account: {}",
        stake_account
    );
...
pub async fn instant_unstake_stake_account(
    ...
    println!(
        "INSTANT UNSTAKE: Starting for stake account: {}",
        stake_account.pubkey
    );
    ...
    let instant_unstake_ix =
        build_instant_unstake_instruction(&user_pubkey, &stake_account.pubkey)?;
    ...
    println!(
        "Sending instant unstake transaction ({} bytes)",
        encoded.len()
    );
    ...
    match transaction_client.send_transaction(&encoded).await {
        Ok(sig) => {
            ...
            println!(
                "Explorer: https://explorer.solana.com/tx/{}?cluster=mainnet",
                sig
            );
            Ok(sig)
        }
...
fn build_deactivate_stake_instruction(
    ...
    println!(
        "Building deactivate instruction for stake account: {}",
        stake_account
    );
...
pub async fn normal_unstake_stake_account(
    ...
    println!(
        "NORMAL UNSTAKE: Starting for stake account: {}",
        stake_account.pubkey
    );
    ...
    println!(
        "Sending normal unstake (deactivate) transaction ({} bytes)",
        encoded.len()
    );
...
pub async fn partial_unstake_stake_account(
    ...
    println!(
        "PARTIAL UNSTAKE: Starting for stake account: {}",
        stake_account.pubkey
    );
    ...
    println!(
        "  Current staked: {:.6} SOL",
        current_staked as f64 / 1_000_000_000.0
    );
    println!(
        "  Will remain: {:.6} SOL",
        remaining_balance as f64 / 1_000_000_000.0
    );
    ...
    println!(
        "Sending partial unstake transaction ({} bytes)",
        encoded.len()
    );
...
pub async fn withdraw_stake_account(
    ...
    println!(
        "WITHDRAW: Starting for stake account: {}",
        stake_account.pubkey
    );
    ...
    println!(
        "Total withdrawable: {:.6} SOL (includes rent reserve)",
        withdraw_amount_sol
    );

    if withdraw_amount == 0 {
        return Err(StakingError::InvalidAmount(
            "Stake account has zero balance".to_string(),
        ));
    }
