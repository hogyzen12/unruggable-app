...
fn parse_jupiter_price_response(
    response_text: &str,
) -> Result<HashMap<String, JupiterTokenPrice>, String> {
    if let Ok(map_response) =
        serde_json::from_str::<HashMap<String, JupiterTokenPrice>>(response_text)
    {
    ...
    serde_json::from_str::<JupiterPriceResponseEnvelope>(response_text)
        ...
        .map_err(|e| {
            format!(
                "Failed to parse Jupiter response: {} - Response: {}",
                e, response_text
            )
        })
}
...
static PRICE_CACHE: OnceLock<
    Mutex<(
        HashMap<String, f64>,
        HashMap<String, MultiTimeframePriceData>,
        Instant,
    )>,
> = OnceLock::new();

fn get_price_cache() -> &'static Mutex<(
    HashMap<String, f64>,
    HashMap<String, MultiTimeframePriceData>,
    Instant,
)> {
...
pub async fn get_jupiter_prices_for_mints(
    ...
    println!("Jupiter price fetch complete: {} prices", prices.len());
    Ok(prices)
}
...
pub async fn get_prices_for_tokens(
    token_mint_to_symbol: HashMap<String, String>,
) -> Result<HashMap<String, f64>, Box<dyn Error>> {
    println!(
        "Fetching prices for {} discovered tokens...",
        token_mint_to_symbol.len()
    );

    // Always include SOL
    let mut all_mints = vec!["So11111111111111111111111111111111111111112".to_string()];
    let mut symbol_to_mint = HashMap::new();
    symbol_to_mint.insert(
        "SOL".to_string(),
...
pub async fn get_token_metadata(
    mint_addresses: Vec<String>,
) -> Result<HashMap<String, JupiterTokenInfo>, Box<dyn Error>> {
    if mint_addresses.is_empty() {
        return Ok(HashMap::new());
    }
    ...
    println!(
        "Fetching token metadata from Jupiter Token API for {} tokens...",
        unique_mints.len()
    );
    ...
    for chunk in chunks {
        ...
        println!(
            "Jupiter Token API request: {} with query: {}",
            JUPITER_TOKEN_API_URL, ids_param
        );
        ...
        let response_text = response.text().await?;
        println!(
            "Jupiter Token API raw response (first 500 chars): {}",
        ...
        let token_infos: Vec<JupiterTokenInfo> =
            serde_json::from_str(&response_text).map_err(|e| {
                format!(
                    "Failed to parse Jupiter Token API response: {} - Response: {}",
                    e, response_text
                )
            })?;
    ...
    println!(
        "Jupiter Token API returned metadata for {} tokens",
        all_tokens.len()
    );
    Ok(all_tokens)
}

fn create_dummy_multi_data(
...
pub async fn get_cached_prices_and_changes() -> Result<
    (
        HashMap<String, f64>,
        HashMap<String, MultiTimeframePriceData>,
    ),
...
> {
    // Check cache first
    {
        ...
        if timestamp.elapsed() < Duration::from_secs(PRICE_CACHE_TIMEOUT)
            && !current_prices.is_empty()
        {
    ...
    println!(
        "Updated price cache with fresh data: {} tokens",
        current_prices.len()
    );
...
pub async fn get_enhanced_cached_prices_and_changes() -> Result<
    (
        HashMap<String, f64>,
        HashMap<String, MultiTimeframePriceData>,
    ),
        ...
        match get_jupiter_prices().await {
            Ok(prices) => {
                ...
                if let Some(sol_price) = prices.get("SOL") {
                    assert!(
                        *sol_price > 10.0 && *sol_price < 1000.0,
                        "SOL price should be reasonable"
                    );
                }
            }
            Err(e) => {
